...
const (
	...
	defaultConfigFilename   = "qitwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "qitwallet.log"
...
var (
	btcdDefaultCAFile  = filepath.Join(util.AppDataDir("qitmeer", false), "rpc.cert")
	defaultAppDataDir  = util.AppDataDir("qitwallet", false)
...
type config struct {
	...
	// Deprecated options
	DataDir *util.ExplicitString `short:"b" long:"datadir" default-mask:"-" description:"DEPRECATED -- use appdata instead"`
}
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		CAFile:                 util.NewExplicitString(""),
		RPCKey:                 util.NewExplicitString(defaultRPCKeyFile),
		RPCCert:                util.NewExplicitString(defaultRPCCertFile),
		LegacyRPCMaxClients:    defaultRPCMaxClients,
		LegacyRPCMaxWebsockets: defaultRPCMaxWebsockets,
		DataDir:                util.NewExplicitString(defaultAppDataDir),
		UseSPV:                 false,
		AddPeers:               []string{},
		ConnectPeers:           []string{},
	}
	...
	if cfg.PrivNet {
		activeNet = &netparams.PrivNetParams
		numNets++
	}
	if numNets > 1 {
	...
	dbFileExists, err := util.FileExists(dbPath)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
	...
	if cfg.CreateTemp {
		...
		keystoreExists, err := util.FileExists(keystorePath)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
		}
	...
	if cfg.UseSPV {
		...
		if cfg.RPCConnect == "" {
			cfg.RPCConnect = net.JoinHostPort("localhost", activeNet.DefaultPort)
		}

		// Add default port to connect flag if missing.
		cfg.RPCConnect, err = util.NormalizeAddress(cfg.RPCConnect,
		...
		if cfg.DisableClientTLS {
			...
			if !cfg.CAFile.ExplicitlySet() {
				...
				certExists, err := util.FileExists(cfg.CAFile.Value)
				if err != nil {
					fmt.Fprintln(os.Stderr, err)
					return nil, nil, err
				}
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						btcdCertExists, err := util.FileExists(
							btcdDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
	...
	if len(cfg.ExperimentalRPCListeners) == 0 && len(cfg.LegacyRPCListeners) == 0 {
		...
		for _, addr := range addrs {
			addr = net.JoinHostPort(addr, activeNet.DefaultPort)
	...
	// duplicate addresses.
	cfg.LegacyRPCListeners, err = util.NormalizeAddresses(
		cfg.LegacyRPCListeners, activeNet.DefaultPort)
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"Invalid network address in legacy RPC listeners: %v\n", err)
		return nil, nil, err
	}
	cfg.ExperimentalRPCListeners, err = util.NormalizeAddresses(
		cfg.ExperimentalRPCListeners, activeNet.DefaultPort)
	if err != nil {
		fmt.Fprintf(os.Stderr,
	...
	if cfg.QitmeerUsername == "" {
		cfg.QitmeerUsername = cfg.Username
	}
	if cfg.QitmeerPassword == "" {
		cfg.QitmeerPassword = cfg.Password
	}
